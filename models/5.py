...
class MorphologyMemoryPool(nn.Module):
    ...
    def _predict_with_fixed_patterns(self, morph0_features):
        ...
        # 选择top-3模式
        top_similarities, top_indices = torch.topk(similarity_matrix, k=5, dim=1)
        
        # 2. 使用注意力机制聚合模式
        selected_patterns = torch.zeros(batch_size, 5, feature_dim, 
    ...
    def update_evolution_patterns(self, morph0_features, morph1_features, learning_rate=0.01):
        ...
        with torch.no_grad():
            # 保证输入为2D
            if morph0_features.dim() == 1:
                morph0_features = morph0_features.unsqueeze(0)
            if morph1_features.dim() == 1:
                morph1_features = morph1_features.unsqueeze(0)
            # 计算演化向量
            ...
            if class_mask.sum() > 0:
                ...
                if self._morphology_stage == 1:
                    ...
                    if 0 in self._morphology_prototypes[self._current_morphology_pair][class_idx]:
                        ...
                        # 更新Memory Pool的演化历史
                        self.memory_pool.update_evolution_patterns(morph0_proto, morph1_proto)
                    ...
                    if hasattr(self, '_morphology_prototypes') and self._current_morphology_pair in self._morphology_prototypes:
                        if class_idx in self._morphology_prototypes[self._current_morphology_pair]:
                            ...
                            if 0 in stages and self._morphology_stage == 1:
                                ...
                                # 更新演化历史
                                self.memory_pool.update_evolution_patterns(
                                    stages[0], new_morph1
                                )
                ...
                for class_idx, stages in pair_prototypes.items():
                    ...
                    if selected_stage is not None:
                        ...
                        if prototype is not None:
                            all_prototypes.append((prototype, class_idx))
        if len(all_prototypes) > max_rehearsal_prototypes:
            import random
            selected_indices = random.sample(range(len(all_prototypes)), max_rehearsal_prototypes)
            all_prototypes = [all_prototypes[i] for i in selected_indices]
        if all_prototypes:
            prototypes_batch = torch.stack([proto for proto, _ in all_prototypes]).to(self._device)
            class_indices = [class_idx for _, class_idx in all_prototypes]
            try:
                class_to_label = self.data_manager._class_to_label
                templates = self.data_manager._data_to_prompt[0]
                total_labels = class_to_label[:self._total_classes]
                text_batch = [templates.format(lbl) for lbl in total_labels]
                proto_logits = self._forward_for_classification_features(prototypes_batch, text_batch)
                ...
                for i, class_idx in enumerate(class_indices):
                    if class_idx < proto_logits.size(1):
                        ...
                        proto_loss = F.cross_entropy(proto_logits[i:i+1], target)
